        ...
        <ul className="mt-14 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {SERVICES.map((service, i) => (
            <Reveal as="li" key={service.title} delay={(i % 3) * 90}>
              <article className="group relative flex h-full flex-col overflow-hidden rounded-xl border border-white/10 bg-ink/60 p-7 transition-all duration-300 hover:-translate-y-1.5 hover:border-brand/40 hover:bg-ink hover:shadow-[0_24px_60px_-24px_rgba(0,0,0,0.7)]">
                <span
                  className="absolute inset-x-0 top-0 h-px scale-x-0 bg-gradient-to-r from-transparent via-brand to-transparent transition-transform duration-500 group-hover:scale-x-100"
                  aria-hidden
                />
                <span
                  ...
                  aria-hidden
                />
                <span className="flex size-12 items-center justify-center rounded-lg border border-white/10 bg-brand-soft text-brand transition-colors duration-300 group-hover:border-brand/50">
